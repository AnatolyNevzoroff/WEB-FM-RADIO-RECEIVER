...
uint8_t chanFM,chanFM_Idx,chanFM_MOD,band=2,INPUT_band=2,BD_menu,R_menu,R_menu_MOD,wait,factor;//
uint8_t triangle,arrow,antenna,pointer,multiline,signvsnoise,logostereo;//СИМВОЛЫ
uint8_t rssi,mixing,signoise,disableAgc,multipath,Internet,Connect;//
...
FL_start_MENU=true;BD_menu=false;FAST_menu=false;pointer=triangle;
...
band=1;input=0;gain=gain1;BD_audio();showVolWEB();
...
FL_runtext=false;//ЗАПРЕЩАЕМ БЕГУЩИЙ ТЕКСТ
WEBradioPRT();//ПЕЧАТАЕМ НОМЕР КАНАЛА, ПУСТОЙ БИТРЕЙТ, И "Connection..."
break;

//МЕНЯЕМ ГРОМКОСТЬ
//  case 1:volWEB=constrain(trig0,0,21);audio.setVolume(volWEB);break;}
  case 1:volume=constrain(trig0,-79,15);bd.setVol(volume);showVolWEB();break;}
}
...
StartSI4735();//ЗАПУСКАЕМ РАДИОМОДУЛЬ  delay(1000);rx.radioPowerUp();
FAST_menu=false;BD_menu=false;
band=2;input=1;gain=gain2;BD_audio();clearRDS();
showChanFM();showFreqFM();showVolFM();timer_MENU=mill;
...
FL_delay_SW=true;timer_BTN=mill;//ПОДНИМАЕМ ФЛАГ КНОПКИ, ОБНУЛЯЕМ ТАЙМЕР
}SW_btn_status=SW_btn;}
...
//ВЫБИРАЕМ НОМЕР КАНАЛА ДЛЯ ЗАПИСИ ТЕКУЩЕЙ ЧАСТОТЫ
...
if(FL_com2==true){
switch(smenu9){
  case 0: volume=constrain(trig9,-79,15);trig9=volume;break;
  case 1: gain3=constrain(trig9,0,20);gain=gain3;trig9=gain3;break;}
...
switch(smenu2){
  ...
  case 1: gain1=constrain(trig2,0,20);trig2=gain1;break;
  case 2: volFM=constrain(trig2,0,63);trig2=volFM;rx.setVolume(volFM);break;
  case 3: gain2=constrain(trig2,0,20);trig2=gain2;break;}
BD_audio();BD_menu2();
   FL_com2=false;}
...
switch(band){
  case 1:WEBradioPRT();showVolWEB();break;
...
//В ЗАВИСИМОСТИ ОТ ПОДКЛЮЧЕНИЯ ВОЗМОЖНО 2 ВАРИАНТА 
switch(Connect){
...
//ЕСЛИ НЕТ ПОДКЛЮЧЕНИЯ, МЕНЯЕМ ТОЧКУ ДОСТУПА НА № 2 
...
{lcd.setCursor(0,1);lcd.print("NO WIFI CONNECT!");}break;
