...
rx.setVolume(volFM);//ПЕРЕДАЁМ УРОВЕНЬ ГРОМКОСТИ В ДИАПАЗОНЕ 0...63
...
if(rssi!=aux||mixing!=mo_st){rssi=aux;mixing=mo_st;//ЕСЛИ ЗНАЧЕНИЯ ИЗМЕНИЛИСЬ

lcd.setCursor(8,0);
if(!rx.getCurrentPilot()){lcd.print("MON");}else{
if(mixing==100){lcd.print("STE");}//ПОЛНОЕ СТЕРЕО
else{lcd.write(logostereo);lcd.print(mixing);if(mixing<10){lcd.print('%');}}}
//if(!rx.getCurrentPilot()){lcd.setCursor(8,0);lcd.print("MON");}
...
//lcd.setCursor(0,1);lcd.print("NTP server LOST!");printLocalTime();
