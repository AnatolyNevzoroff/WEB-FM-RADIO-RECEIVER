...
if(chanWEB<10){lcd.print(' ');}lcd.print(chanWEB);
#ifdef LOG_ENABLE
Serial.println("PRINT NAMBER WEB STATION");
#endif
}


//БИТРЕЙТ ВЕЩАНИЯ СТАНЦИИ
void ShowWEBbitrate(){
lcd.setCursor(4,0);
//bitrate.toInt();lcd.print(float(bitrate.toInt()/1000),0);//ВАРИАНТ С float ПЕРЕМЕННОЙ
bitrate.remove(3);if(bitrate==""){lcd.print("   ");}
lcd.print(bitrate);lcd.print('k');lcd.print(' ');
#ifdef LOG_ENABLE
Serial.println("PRINT BITRATE WEB STATION");
#endif
}


//НАЗВАНИЕ СТАНЦИИ ИЛИ ИМЯ АРТИСТА И НАЗВАНИЕ КОМПОЗИЦИИ
void ShowWEBinfoLINE(){
lcd.setCursor(0,1);
lcd.print(infoLINE);
#ifdef LOG_ENABLE
Serial.println("PRINT WEB INFORMATION LINE");
#endif
}
...
//ФУНКЦИЯ ПЕЧАТИ НА ЭКРАНЕ УРОВНЯ ГРОМКОСТИ
void ShowVolWEB(){
lcd.setCursor(9,0);
...
void StartSI4735(){
rx.radioPowerUp();//ПОДАЁМ ПИТАНИЕ
rx.setup(RESET_PIN, FM_BAND_TYPE);//ЗАПУСКАЕМ SI4735 ТОЛЬКО В РЕЖИМЕ FM
rx.setI2CFastModeCustom(100000);//СКОРОСТЬ ШИНЫ I2C
rx.getDeviceI2CAddress(RESET_PIN);//ПО I2C УСТАНАВЛИВАМ АДРЕС. ЕСЛИ НЕТ ПОЛУЧАЕМ "0"
rx.setTuneFrequencyAntennaCapacitor(0);//8 ПИН ДЛЯ FM (ЗНАЧЕНИЕ ВХОДНОГО КОНДЕНСАТОРА АВТО)
rx.setFM(6400,10800,10110,10);//НИЖНЯЯ И ВЕРХНЯЯ ГРАНИЦЫ ЧАСТОТ, ЧАСТОТА, ШАГ 100кГц
//rx.setSeekFmLimits(6400, 10800);//ГРАНИЦЫ АВТОПОИСКА rx.setFmStereoOn();rx.setFmStereoOff();
...
rx.setVolume(volFM);//ЗАДАЁМ УРОВЕНЬ ГРОМКОСТИ В ДИАПАЗОНЕ 0...63
rx.setFmBandwidth(bandwidth);//ЗАДАЁМ ШИРИНУ ПОЛОСЫ 0...4
rx.setFmSoftMuteMaxAttenuation(softMute);//ПРОГРАММНОЕ СНИЖЕНИЕ ШУМОВ 0...30
rx.setFMDeEmphasis(Emphasis);//ПРЕДИСКАЖЕНИЯ 1...2 (ДЛЯ ЕВРОППЫ 1 - 50 μs. ДЛЯ США 2 - 75 μs.)
...
void ShowChanFM(){
lcd.setCursor(0,1);
if(smenu8==0||smenu8==3){lcd.write(pointer);}else{lcd.print(' ');}
...
//lcd.print((float)freqFM/100,1);//ВАРИАНТ С float
...
void ShowSTEREO_RSSI(){
rx.getCurrentReceivedSignalQuality();//ЭТА ФУНКЦИЯ ДОЛЖНА БЫТЬ ВЫЗВАНА САМОЙ ПЕРВОЙ
int CurRSSI=rx.getCurrentRSSI();
int CurSTEREO=rx.getCurrentStereoBlend();
//ЕСЛИ ЗНАЧЕНИЯ ИЗМЕНИЛИСЬ
if(ShowRSSI!=CurRSSI||ShowSTEREO!=CurSTEREO){ShowRSSI=CurRSSI;ShowSTEREO=CurSTEREO;
//СОСТОЯНИЕ СТЕРЕО
lcd.setCursor(8,0);
if(!rx.getCurrentPilot()){lcd.print("MON");}else{//ПИЛОТ-ТОН "СТЕРЕО" НЕ ОБНАРУЖЕН
if(ShowSTEREO==100){lcd.print("STE");}else{//ПОЛНОЕ СТЕРЕО
lcd.write(logostereo);lcd.print(ShowSTEREO);if(ShowSTEREO<10){lcd.print('%');}}}//СТЕРЕО В %
//УРОВЕНЬ СИГНАЛА
lcd.setCursor(13,0);
lcd.write(antenna);if(ShowRSSI<10){lcd.print(' ');}lcd.print(ShowRSSI);}

#ifdef LOG_ENABLE
Serial.print("УРОВЕНЬ СИГНАЛА ");Serial.print(ShowRSSI);Serial.println(" dBuV");
Serial.print("МОНО / СТЕРЕО ");Serial.print(ShowSTEREO);Serial.println(" %");
#endif
}
...
void ShowSNR_MULTI(){
int CurSNR=rx.getCurrentSNR();//SNR (dB)
int CurMULTI=rx.getCurrentMultipath();//МНОГОЛУЧЕВЫЕ ПОМЕХИ (ОТРАЖЁННЫЕ СИГНАЛЫ В %)
//int CurMULTI=rx.getCurrentSignedFrequencyOffset();//СМЕЩЕНИЕ В КГЦ (МЕНЕЕ ИНТЕРЕСНЫЙ ПОКАЗАТЕЛЬ)
if(ShowSNR!=CurSNR||ShowMULTI!=CurMULTI){ShowSNR=CurSNR;ShowMULTI=CurMULTI;

//СИГНАЛ / ШУМ (SNR)
lcd.setCursor(8,1);
lcd.write(signvsnoise);if(ShowSNR<10){lcd.print(' ');}lcd.print(ShowSNR);

//МНОГОЛУЧЕВЫЕ ПОМЕХИ
lcd.setCursor(13,1);
lcd.write(multiline);
if(ShowMULTI==100){lcd.write(multiline);lcd.write(multiline);}else{
lcd.print(ShowMULTI);if(ShowMULTI<10){lcd.print('%');}}}

#ifdef LOG_ENABLE
Serial.print("СИГНАЛ / ШУМ ");Serial.print(ShowSNR);Serial.println(" dB");
#endif
}
...
//lcd.print('V');if(volFM<10){lcd.print(' ');}lcd.print(volFM);//ГРОМКОСТЬ SI4735
lcd.print('V');if(volume<-74){lcd.print(' ');}lcd.print(volume+84);//ГРОМКОСТЬ BD37534FV
}
...
if(smenu1==0){
lcd.setCursor(0,0);lcd.write(pointer);lcd.setCursor(0,1);lcd.write(pointer);}else{
lcd.setCursor(0,0);lcd.print(' ');lcd.setCursor(0,1);lcd.print(' ');}
if(bass>0){lcd.print('+');}if(bass==0){lcd.print(' ');}lcd.print(bass);lcd.print(' ');

if(smenu1==1){
lcd.setCursor(6,0);lcd.write(pointer);lcd.setCursor(6,1);lcd.write(pointer);}else{
...
if(smenu1==2){
lcd.setCursor(12,0);lcd.write(pointer);lcd.setCursor(12,1);lcd.write(pointer);}else{
lcd.setCursor(12,0);lcd.print(' ');lcd.setCursor(12,1);lcd.print(' ');}
...
//ФУНКЦИЯ ПЕЧАТИ НАСТРОЕК ЭКВАЛАЙЗЕРА
void ShowBDmenu_3(){
//ЦЕНТРАЛЬНАЯ ЧАСТОТА В ДИАПАЗОНЕ "BASS"
lcd.setCursor(0,0);
...
lcd.print("0 Hz");
//ДОБРОТНОСТЬ ДЛЯ ВЫБРАННОЙ ЧАСТОТЫ (Q-FACTOR)
lcd.setCursor(0,1);
...
//ФУНКЦИЯ ПЕЧАТИ НАСТРОЕК ЭКВАЛАЙЗЕРА
void ShowBDmenu_4(){
//ЦЕНТРАЛЬНАЯ ЧАСТОТА В ДИАПАЗОНЕ "MIDDLE"
lcd.setCursor(0,0);
if(smenu4==0){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("MIDDLE ");
  switch(mid_c){
...
//ДОБРОТНОСТЬ ДЛЯ ВЫБРАННОЙ ЧАСТОТЫ (Q-FACTOR)
lcd.setCursor(0,1);
if(smenu4==1){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print(text_Q);
  switch(mid_q){
...
//ФУНКЦИЯ ПЕЧАТИ НАСТРОЕК ЭКВАЛАЙЗЕРА
void ShowBDmenu_5(){
//ЦЕНТРАЛЬНАЯ ЧАСТОТА В ДИАПАЗОНЕ "TREBLE"
lcd.setCursor(0,0);
if(smenu5==0){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("TREBLE ");
  switch(treb_c){
...
//ДОБРОТНОСТЬ ДЛЯ ВЫБРАННОЙ ЧАСТОТЫ (Q-FACTOR)
lcd.setCursor(0,1);
if(smenu5==1){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print(text_Q);
  switch(treb_q){
    case 0: lcd.print("0.75");break;
...
void ShowBDmenu_6(){
//ЛЕВЫЙ КАНАЛ ДЛЯ ЛИНЕЙНОГО ВЫХОДА
lcd.setCursor(0,0);
if(smenu6==0){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("LSP");
lcd.setCursor(4,0);
if(lf>0){lcd.print('+');}if(lf==0){lcd.print(' ');}lcd.print(lf);lcd.print(' ');
//ПРАВЫЙ КАНАЛ ДЛЯ ЛИНЕЙНОГО ВЫХОДА
lcd.setCursor(9,0);
if(smenu6==1){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("RSP");
lcd.setCursor(13,0);
if(rf>0){lcd.print('+');}if(rf==0){lcd.print(' ');}lcd.print(rf);lcd.print(' ');
//ЛЕВЫЙ КАНАЛ ДЛЯ ВЫХОДА НА НАУШНИКИ
lcd.setCursor(0,1);
if(smenu6==2){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("LHP");
lcd.setCursor(4,1);
if(lt>0){lcd.print('+');}if(lt==0){lcd.print(' ');}lcd.print(lt);lcd.print(' ');
//ПРАВЫЙ КАНАЛ ДЛЯ ВЫХОДА НА НАУШНИКИ
lcd.setCursor(9,1);
if(smenu6==3){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("RHP");
lcd.setCursor(13,1);
...
//ФУНКЦИЯ ПЕЧАТИ НАСТРОЕК LOUDNESS, КОЛЛИЧЕСТВО СТАНЦИЙ В ПАМЯТИ, ОЖИДАНИЯ АВТОВОЗВРАТА ИЗ МЕНЮ
void ShowBDmenu_7(){
//УРОВЕНЬ ФОРМИРОВАНИЯ АЧХ ДЛЯ РЕЖИМА LOUDNESS
lcd.setCursor(0,0);
if(smenu7==0){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("UP ");
...
//МАКСИМАЛЬНОЕ ЧИСЛО ЯЧЕЕК ПАМЯТИ ДЛЯ ЗАПИСИ ЧАСТОТ РАДИОСТАНЦИЙ (5...50)
lcd.setCursor(0,1);
if(smenu7==2){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("FMch ");
if(maxFMchan<10){lcd.print(' ');}lcd.print(maxFMchan);
//ЗАДЕРЖКА ПЕРЕД АВТОВОЗВРАТОМ В ОСНОВНОЕ МЕНЮ В СЕКУНДАХ 
lcd.setCursor(9,1);
if(smenu7==3){lcd.write(pointer);}else{lcd.print(' ');}
lcd.print("RET ");
if(wait<10){lcd.print(' ');}lcd.print(wait);
...
#ifdef LOG_ENABLE
Serial.println("МЕНЮ ШИРИНА ПОЛОСЫ, ШУМЫ, ПРЕДИСКАЖЕНИЯ, 64/87");
#endif
}//END 2 MENU SI4735
...
void ConnectNTP(){
getLocalTime(&timeinfo);delay(500);
//while(!getLocalTime(&timeinfo)){ 
for(byte b=0;b<5;b++){if(getLocalTime(&timeinfo)){b=5;Internet=1;lcd.clear();}
else{lcd.setCursor(0,1);lcd.print("Connection NTP ");lcd.print(b);Internet=0;}}
//lcd.setCursor(0,1);lcd.print("NTP server LOST!");ShowLocalTime();

#ifdef LOG_ENABLE
Serial.print("ДАТА И ВРЕМЯ ПОЛУЧЕНЫ ");Serial.println(Internet);
#endif
}
...
void ShowLocalTime(){
getLocalTime(&timeinfo);//ЗАПРАШИВАЕМ ВРЕМЯ
...
lcd.setCursor(0,1);lcd.print("Connected ");lcd.print(b);delay(500);
if(WiFi.status()!=WL_CONNECTED){delay(500);Connect=0;}else
{b=5;Connect=1;WiFi.mode(WIFI_STA);WiFi.setSleep(false);lcd.clear();
//ПЕЧАТАЕМ АДРЕС ПОДКЛЮЧЕНИЯ
lcd.setCursor(0,0);
...
void EEPROM_Update(){
if(ChanUpd==true){
for(byte b=0;b<maxFMchan+1;b++){EEPROM.write(0+b,highByte(channel[b]));
...
EEPROM.write(186,wait);EEPROM.write(189,Loud);EEPROM.write(187,Loud_f);EEPROM.write(157,maxFMchan);
   BDmenu7_save=false;}
if(R_menu!=R_menu_MOD){EEPROM.write(175,R_menu);R_menu_MOD=R_menu;}
if(BDmenu345_save==true){EEPROM.write(176,treb_c);EEPROM.write(177,mid_c);EEPROM.write(178,bas_c);
                 EEPROM.write(180,treb_q);EEPROM.write(181,mid_q);EEPROM.write(182,bas_q);
   BDmenu345_save=false;}
...
volume=-29;volume_MOD=0;treb=0;middle=0;bass=0;chanWEB=1;chanWEB_MOD=0;chanFM=1;chanFM_MOD=0;
volFM=63;volWEB=15;LCD_led=1;LCD_led_MOD=0;maxFMchan=20;
...
BDmenu1_save=true;BDmenu2_save=true;BDmenu6_save=true;BDmenu7_save=true;BDmenu345_save=true;
}
...
//ЦИКЛ ПЕРЕЗАПИСИ 50 ЧАСТОТ В EEPROM, АДРЕСА ЯЧЕЕК (highByte С 0 ПО 49, lowByte С 50 ПО 99)
//void Channel_Update(){
//for(byte b=0;b<maxFMchan+1;b++){EEPROM.write(0+b,highByte(channel[b]));
//                                EEPROM.write(50+b,lowByte(channel[b]));}
//#ifdef LOG_ENABLE
...
//channelNAME="Connection . . .";infoLINE=channelNAME;bitrate="";// Read Information
//ПЕЧАТАЕМ НОМЕР КАНАЛА, ПУСТОЙ БИТРЕЙТ, И "Connection . . ."
//ShowWEBbitrate();ShowWEBinfoLINE();

//||chanWEB!=OLDchanWEB&&channelNAME!="Connection . . ."){OLDchanWEB=chanWEB;
//lcd.setCursor(0,1);lcd.print(emptyBLANC);//ПЕЧАТАЕМ ПРОБЕЛЫ ЗАТИРАЯ СТАРУЮ ИНФОРМАЦИЮ
//OLDartistTITLE="";//ОБНУЛЯЕМ OLDartistTITLE ЧТОБ ВЫПОЛНЯЛОСЬ УСЛОВИЕ artistTITLE!=OLDartistTITLE И
...
//infoLINE=artistTITLE;//ПЕРЕМЕННОЙ ИНФОСТРОКИ ПРИСВАИВАЕМ ПОЛНОЕ ИМЯ ИСПОЛНИТЕЛЯ
//ЕСЛИ ИМЯ ИСПОЛНИТЕЛЯ ПРЕВЫШАЕТ КОЛИЧЕСТВО ЗАДАННЫХ СИМВОЛОВ, ОБРЕЗАЕМ ЕГО
//if(artistTITLE.length()>sizeL){infoLINE.remove(sizeL);}ShowWEBinfoLINE();
